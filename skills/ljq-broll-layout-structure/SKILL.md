...
1. 在关键帧中选择一个“主要入场已完成、退出尚未开始、信息关系最完整”的落定帧。不是机械中点。证据不足时可补抽相邻全分辨率帧。
2. 把画面拆成真正需要独立定位、替换、遮挡或后续运动的元素。为它们创建稳定、小写连字符 ID；后续 Skill 不得重命名。
3. 测量百分比边界 `[left, top, width, height]`、锚点、父组、层级和裁切。越界元素保留真实越界关系。
4. 先判断元素是完整素材、可见区域提取，还是仅由容器/画布裁切。需要提取时保存到案例 `assets/extracted/`，记录来源和完整性；不做生成式补全。
5. 记录颜色、透明度、混合模式、滤镜、模糊、阴影、圆角、媒体适配和字体占位。无法唯一识别字体或混合方式时标为 `inferred` 或 `default`，不能伪装成测量值。
...
8. 用 Remotion 渲染落定帧，与原始落定帧并排或叠放校准。优先修正画布/裁切、主体边界、层级、留白、文字占位，再修表面样式。
9. 更新 `case.json.files.layout`、`composition`、`propsSchema`，把 `stages.layout` 设为 `passed`。运行主 Skill 的 `scripts/validate-case.mjs <case-dir>`。
...
- 字体无法从压平视频被唯一确认。以字宽、行高、换行、字重和视觉占位匹配为验收目标，并记录候选字体。
