...
def main() -> None:
    ...
    if dimensions_equal:
        subprocess.run([sys.executable, str(compare_script), str(source), str(render), "--output", str(evidence_directory)], check=True)
    ...
    def check(check_id: str, passed: bool, evidence: str) -> None:
        ...
        if not passed:
            issues.append({
                "id": f"issue-{check_id}", "type": "audio" if check_id == "audio" else "render",
                "targetId": None, "owner": "implementation", "severity": "high",
    ...
    report = {
        "schemaVersion": "1.1", "caseId": state["caseId"], "status": status,
