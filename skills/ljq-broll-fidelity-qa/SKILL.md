...
1. 读取 `case.json`、layout、motion、完整 render 和 [references/quality-gate.md](references/quality-gate.md)；`implementation` 必须为 `passed`。判断静帧比例、配色/材质、蒙版或替换结果前，必须读取布局 owner 的 [static-compositing.md](../ljq-broll-layout-structure/references/static-compositing.md)。
...
3. **核对排除区域。** 有 `scope.exclude` 时，按 `excludedRegionMode` 验证空白/透明/底色，并确认 `exclusionMask` 只排除用户明确不要的区域。时变口播/穿字转场、工具不支持其mask或用户已认可但自动门未过时，必须按[时变排除与能力限制](references/quality-gate.md#时变排除与能力限制)保留真实状态。
...
6. **写入视觉结论。** 通过时运行：
   ...
   明显偏差使用 `--visual-status fail --owner preflight|layout|motion|implementation|qa --observation "..."`，每次只指定一个最高影响根因或同根因问题组。该命令会写状态；总控还须做`--complete`校验，不以命令退出码宣布完成。
