...
一个用户指定的连续范围可以包含多个镜头/排版场景，同一案例用scene和交叠区表达；切点不是擅自缩短范围或另建案例的授权。人物排除色按本次要求，纯白只是可选审阅底，不是所有案例默认。

QA只能在现有工具准确支持遮罩时计算范围内差异。排除区随场景或穿字转场变化时，读取[时变排除与能力限制](../../ljq-broll-fidelity-qa/references/quality-gate.md#时变排除与能力限制)，保留待审而不是使用占位mask。
